...
/// - **Exploratory** (`environments`, `maxMiles`) — surfaced in the sheet to
///   gauge whether they're worth keeping, but NOT present on the model yet, so
///   they intentionally don't filter anything. Promote them onto `SpotSummary`
///   (and into `matches(_:)`) once the backend exposes the data.
nonisolated struct SpotFilters: Equatable {
    ...
    static let environmentOptions = ["Forest", "Coast", "Mountain", "Desert", "Urban", "Waterfall"]
    ...
    var activeCount: Int {
        ...
        if !environments.isEmpty { n += 1 }
        if maxMiles != nil { n += 1 }
        return n
    }

    /// Whether `spot` passes the model-backed filters. Exploratory filters
    /// (`environments`, `maxMiles`) are intentionally ignored — no data yet.
