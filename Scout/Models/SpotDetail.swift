...
nonisolated struct SpotDetail: Identifiable, Decodable, Hashable {
    ...
    /// Review-distilled blurb generated by the backend. `nil` until a spot has
    /// ≥3 reviews and generation has run, so the UI hides its section when absent.
    /// Detail-only — not present on `SpotSummary` (list/nearby/search payloads).
...
nonisolated extension SpotDetail {
    static let sample = SpotDetail(
        ...
        aiSummary: "A serene alpine basin that rewards an early start — most reviewers shoot it at golden hour, using a wide lens and a tripod for the still-water reflections. Expect a moderate hike in and light crowds midweek."
    )
}
